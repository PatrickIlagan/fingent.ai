...
target_regex = r"(\s*\}\)\}\s*<\/div>\s*\)\s*:\s*\(\s*<div\s+className=\{`rounded-3xl shadow-sm border p-6 sm:p-8)"

replacement = r"""\1"""

match = re.search(target_regex, content)
if match:
    # let's just insert before the match
    parts = content.split(match.group(0))
