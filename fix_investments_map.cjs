...
content = content.replace("export default function Investments() {", "export default function Investments() {\n" + mapFn);

content = content.replace(/const formatted = dataList\.map\(\(d: any\) => \(\{[\s\S]*?\}\)\);/g, "const formatted = mapPortfolioData(dataList);");
content = content.replace(/const formatted = data\.map\(\(d: any\) => \(\{[\s\S]*?\}\)\);/g, "const formatted = mapPortfolioData(data);");
