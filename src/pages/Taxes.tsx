...
type Scenario = {
  ...
  nonTaxable: string;
  statutory: string;
  monthlyContributionBase: string;
  useEstimatedContributions: boolean;
...
const initial: Scenario = {
  profile: 'employment', compensation: '', nonTaxable: '', statutory: '', monthlyContributionBase: '', useEstimatedContributions: true,
...
export function Taxes() {
  ...
  const result = useMemo(() => {
    ...
    const monthlyBasis = n(scenario.monthlyContributionBase) || compensation / 12;
    const sssMsc = Math.min(35000, Math.max(5000, Math.round(monthlyBasis / 500) * 500));
    const employeeSssMonthly = sssMsc * .05;
    const philHealthBase = Math.min(100000, Math.max(10000, monthlyBasis));
    const employeePhilHealthMonthly = philHealthBase * .05 / 2;
    const employeePagIbigMonthly = Math.min(monthlyBasis, 5000) * (monthlyBasis <= 1500 ? .01 : .02);
    const estimatedStatutory = (employeeSssMonthly + employeePhilHealthMonthly + employeePagIbigMonthly) * 12;
    const statutory = scenario.useEstimatedContributions && scenario.profile !== 'self-employed' ? estimatedStatutory : n(scenario.statutory);
    const taxableComp = Math.max(0, compensation - n(scenario.nonTaxable) - statutory);
    ...
    return {
      taxableComp, gross, deduction, netBusiness, eligible8, incomeTax, businessTax, monthlyBasis,
      employeeSssMonthly, employeePhilHealthMonthly, employeePagIbigMonthly, estimatedStatutory, statutory,
  ...
  const taxExportRows = [
    { Item: 'Profile', Value: scenario.profile }, { Item: 'Taxable compensation', Value: result.taxableComp },
    { Item: 'Business gross receipts', Value: result.gross }, { Item: 'Income tax', Value: result.incomeTax },
    { Item: result.businessTaxName, Value: result.businessTax }, { Item: 'Credits and withholding', Value: result.credits },
    { Item: 'Estimated annual payable', Value: result.payable }, { Item: 'Quarterly planning reserve', Value: result.payable / 4 }
  ];

  return <div className="space-y-6 pb-10">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-3xl font-black">Philippine Tax Planner</h2><p className="mt-1 text-slate-500">Transparent planning estimates for individual taxpayers in the Philippines.</p></div><div className="flex gap-2"><button onClick={() => exportExcel(`philippine-tax-estimate-${taxYear}`, [{ name: 'Tax Estimate', rows: taxExportRows }])} className={"flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold " + (isAdvanced ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50')}><Download size={16} /> Excel</button><button onClick={() => exportPdf(`Philippine Tax Estimate ${taxYear}`, taxExportRows, 'Planning estimate only - verify before filing.')} className={"flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold " + (isAdvanced ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50')}><FileText size={16} /> PDF</button></div></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
      <section className={"rounded-3xl border p-6 " + card}><div className="mb-6 flex items-center gap-3"><div className={isAdvanced ? 'rounded-xl bg-violet-500/20 p-2.5 text-violet-300' : 'rounded-xl bg-violet-50 p-2.5 text-violet-600'}><Calculator size={20} /></div><div><h3 className="font-black text-lg">Tax scenario</h3><p className="text-sm text-slate-500">Use annual amounts in pesos.</p></div></div>
        <div className="space-y-5">
          ...
          {scenario.profile !== 'self-employed' && <div className="grid gap-3 sm:grid-cols-2"><MoneyField label="Annual compensation" value={scenario.compensation} setValue={(value) => set({ compensation: value })} help="Before employee mandatory contributions." /><MoneyField label="Non-taxable compensation" value={scenario.nonTaxable} setValue={(value) => set({ nonTaxable: value })} help="Only confirmed non-taxable amounts." /><MoneyField label="Monthly contribution salary basis" value={scenario.monthlyContributionBase} setValue={(value) => set({ monthlyContributionBase: value })} help="Use monthly basic/remuneration from payroll. Leave blank to use annual compensation ÷ 12." /><div className="sm:col-span-2"><label className={scenario.useEstimatedContributions ? (isAdvanced ? 'flex cursor-pointer gap-3 rounded-2xl border border-violet-500 bg-violet-500/10 p-4' : 'flex cursor-pointer gap-3 rounded-2xl border border-emerald-500 bg-emerald-50 p-4') : (isAdvanced ? 'flex cursor-pointer gap-3 rounded-2xl border border-slate-700 bg-slate-900/40 p-4' : 'flex cursor-pointer gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4')}><input type="checkbox" checked={scenario.useEstimatedContributions} onChange={(event) => set({ useEstimatedContributions: event.target.checked })} className="mt-1" /><span><strong className="text-sm">Estimate employee statutory contributions</strong><span className="mt-1 block text-xs text-slate-500">Applies employee SSS, PhilHealth, and Pag-IBIG shares only. Employer shares are excluded.</span></span></label>{!scenario.useEstimatedContributions && <div className="mt-3"><MoneyField label="Actual annual employee contributions" value={scenario.statutory} setValue={(value) => set({ statutory: value })} help="Enter your actual payroll contributions if they differ from the estimate." /></div>}</div></div>}
      ...
      <section className="space-y-6"><div className={"rounded-3xl border p-6 " + card}><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Estimated annual tax payable</p><p className="mt-2 text-4xl font-black text-rose-500">{php(result.payable)}</p><p className="mt-2 text-sm text-slate-500">{result.overpayment > 0 ? php(result.overpayment) + ' more credits than this estimate.' : php(result.beforeCredits) + ' tax before credits and payments.'}</p><div className="mt-6 grid gap-3 sm:grid-cols-3">{[['Taxable compensation',php(result.taxableComp)], [scenario.eightPercent ? '8% business base' : 'Net business income',php(scenario.eightPercent ? (scenario.profile === 'mixed' ? result.gross : Math.max(0,result.gross - 250000)) : result.netBusiness)], ['Effective rate',result.effective.toFixed(1) + '%']].map(([label,value]) => <div key={label} className={isAdvanced ? 'rounded-2xl bg-slate-900/60 p-4' : 'rounded-2xl bg-slate-50 p-4'}><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-1 font-black">{value}</p></div>)}</div><div className={isAdvanced ? 'mt-5 rounded-2xl bg-slate-900/60 p-4' : 'mt-5 rounded-2xl bg-slate-50 p-4'}><p className="text-xs font-bold text-slate-500">Simple quarterly reserve</p><p className="mt-1 text-xl font-black">{php(result.payable / 4)}</p><p className="mt-1 text-xs text-slate-500">A planning split only. BIR quarterly returns use cumulative computations and credits.</p></div></div>
        {scenario.profile !== 'self-employed' && <div className={"rounded-3xl border p-6 " + card}><h3 className="font-black text-lg">Employee statutory contributions</h3><p className="mt-1 text-sm text-slate-500">Based on the monthly contribution salary basis of {php(result.monthlyBasis)}. These are planning estimates, not a replacement for your payroll table.</p><div className="mt-5 grid gap-3 sm:grid-cols-3"><Contribution label="SSS employee share" monthly={result.employeeSssMonthly} annual={result.employeeSssMonthly * 12} detail="5% of estimated MSC; ₱5k–₱35k MSC range" advanced={isAdvanced} /><Contribution label="PhilHealth employee share" monthly={result.employeePhilHealthMonthly} annual={result.employeePhilHealthMonthly * 12} detail="Half of 5% premium; ₱10k–₱100k base" advanced={isAdvanced} /><Contribution label="Pag-IBIG employee share" monthly={result.employeePagIbigMonthly} annual={result.employeePagIbigMonthly * 12} detail="1% up to ₱1.5k; otherwise 2%, capped at ₱5k base" advanced={isAdvanced} /></div><p className="mt-4 text-xs text-slate-500">Estimated employee total: <strong>{php(result.estimatedStatutory)}</strong> annually. This calculator does not include employer shares, EC, or special member categories such as kasambahay/OFW/self-employed schedules.</p></div>}
    ...
    <div className="grid gap-6 lg:grid-cols-2"><section className={"rounded-3xl border p-6 " + card}><h3 className="flex items-center gap-2 font-black text-lg"><FileText size={20} /> Planning deadlines</h3><p className="mt-1 text-sm text-slate-500">General statutory dates; taxpayer-specific schedules and extensions can differ.</p><div className="mt-5 space-y-3"><Deadline form={scenario.profile === 'employment' ? '1700 (if required)' : '1701'} label={'Annual income tax return for ' + taxYear} date={'April 15, ' + (taxYear + 1)} advanced={isAdvanced} />{scenario.profile !== 'employment' && !scenario.eightPercent && <Deadline form="1701Q" label="Quarterly income tax return (Q1)" date={'May 15, ' + taxYear} advanced={isAdvanced} />}{scenario.profile !== 'employment' && scenario.businessTax === 'percentage' && !scenario.eightPercent && <Deadline form="2551Q" label="Quarterly percentage tax return (Q1)" date={'April 25, ' + taxYear} advanced={isAdvanced} />}</div></section>
      <section className={"rounded-3xl border p-6 " + card}><h3 className="flex items-center gap-2 font-black text-lg"><Info size={20} /> Official references</h3><p className="mt-1 text-sm text-slate-500">Review these before relying on an election, contribution estimate, or filing position.</p><div className="mt-5 space-y-3"><Reference label="2023 onward individual income-tax brackets" href="https://bir-cdn.bir.gov.ph/BIR/pdf/RMC%20No.%2034-2025%20Annex%20A.pdf" advanced={isAdvanced} /><Reference label="8% option, mixed-income treatment, and ₱3M threshold" href="https://bir-cdn.bir.gov.ph/local/pdf/RMO%20NO.23-2018.pdf" advanced={isAdvanced} /><Reference label="1701Q: 8% option and 40% OSD guidance" href="https://efps.bir.gov.ph/efps-war/EFPSWeb_war/forms2018Version/1701Q/1701q_v3_01.xhtml" advanced={isAdvanced} /><Reference label="SSS contribution table and member guidance" href="https://www.sss.gov.ph/sss-contribution-table/" advanced={isAdvanced} /><Reference label="PhilHealth 5% premium advisory and salary ceiling" href="https://www.philhealth.gov.ph/advisories/2025/PA2025-0002.pdf" advanced={isAdvanced} /><Reference label="Pag-IBIG member contribution guidelines" href="https://www.pagibigfund.gov.ph/document/pdf/circulars/provident/HDMF%20Circular%20No.%20274%20-%20Revised%20Guidelines%20on%20Pag-IBIG%20Fund%20Membership.pdf" advanced={isAdvanced} /></div></section></div>
  </div>;
}
...
function Contribution({ label, monthly, annual, detail, advanced }: { label: string, monthly: number, annual: number, detail: string, advanced: boolean }) {
  return <div className={advanced ? 'rounded-2xl bg-slate-900/60 p-4' : 'rounded-2xl bg-slate-50 p-4'}><p className="text-xs font-bold text-slate-500">{label}</p><p className="mt-1 text-lg font-black">{php(monthly)}<span className="text-xs font-medium text-slate-400"> / mo</span></p><p className="mt-1 text-xs font-bold text-slate-500">{php(annual)} / year</p><p className="mt-2 text-[11px] leading-relaxed text-slate-400">{detail}</p></div>;
}
